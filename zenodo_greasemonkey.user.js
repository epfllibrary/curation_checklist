...
const checklistData = {
  "M1": {"full": "At least one author must be affiliated with EPFL at the time of the submission or creation of the submitted work",
         ...
         "answers": "There is not enough evidence that the authors are or were affiliated with EPFL, we would be grateful for more details (for example an e-mail address or ORCID identifier)",
         "category": "must",
         "wrapper": "div"},                
  ...
  "M3": {"full": "The Description of the submitted dataset must be  sufficiently detailed. Mere references to external articles or other resources are not a sufficient description",
         "answers": "For example, a few sentences explaining how the files were generated or used would be helpful for a potential user. If the data was used in a publication, you could also include part of the article abstract, to make the scientific context more immediately apparent.",
         "category": "must",
  ...
  "R1": {"full": "Authors are identified by their ORCID",
         "answers": "By listing all authors with their respective ORCID, you make sure that they can be recognized unambiguously. If an EPFL author has no ORCID yet, we strongly suggest to create one, see e strongly suggest you to create one: see https://actu.epfl.ch/news/link-your-orcid-profile-with-epfl/ for more info",
  ...
  frm.addEventListener("click", function(event) {
    ...
    if (MustCheckboxBad.length) {
      ...
      MustCheckboxBad.each(function () {
        ...
        mustArray.push([value, checklistData[value].full, checklistData[value].answers]);
      });
      mustArray.sort();
    ...
    if (RecommendedCheckboxBad.length) {
      ...
      RecommendedCheckboxBad.each(function () {
        let value = $(this).parent().attr('id');
        recommArray.push([value, checklistData[value].full, checklistData[value].answers]);
      });
      recommArray.sort();
    ...
    if (NTHCheckboxBad.length) {
      ...
      NTHCheckboxBad.each(function () {
        let value = $(this).parent().attr('id');
        nthArray.push([value, checklistData[value].full, checklistData[value].answers]);
      });
      nthArray.sort();
